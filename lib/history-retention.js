...
function ofacState(result, legacy) {
  ...
  if (result?.status === "skipped" && result.passed === null) return "not_run";
  ...
  if (result?.passed === true) return result.stale ? "stale" : "clear";
  if (!result && legacy === true) return "clear";
...
function repeatState(result, legacy) {
  if (result?.error || result?.status === "error") return "error";
  if (result?.status === "skipped" && result.passed === null) return "not_run";
  if (result?.status === "not_applicable" && result.passed == null) return "na";
  ...
  if (result) {
    if (result.status === "eligible" && result.passed === true) return "eligible";
    if (result.status === "ineligible" && result.passed === false) return "flagged";
  } else if (legacy === true) {
    return "eligible";
  }
  if (!result && legacy === "na") return "na";
  if (result || legacy === false) return "review";
  return "not_run";
}
...
function titleState(result, legacy) {
  ...
  // A failed title check can still report a brand or lien. Preserve those
  // findings on the badge; only an unconfirmed clear result is generic review.
  if (result && result.passed !== true) return "review";
  if (
    result?.passed === true &&
  ...
  if (!result && TITLE_STATES.has(legacy)) return legacy;
...
export function minimizeHistoryEntry(entry) {
  ...
  const legacyResults = entry.savedResults || entry.fullResults || {};
  const resultsChecks = legacyResults.checks || {};
  // Once checks were saved, their snapshot is authoritative. A summarized
  // badge from an older release must not override missing or contradictory data.
  const legacyChecks = legacyResults.checks ? {} : entry.checks || {};
  const id = Number.isFinite(Number(entry.id)) ? Number(entry.id) : time;
  const hasCoBuyer = Boolean(
    entry.hasCoBuyer ||
      legacyResults.customer?.hasCoBuyer ||
      legacyResults.customer?.coBuyer ||
