...
test("camera screen keeps only essential visible guidance", () => {
  ...
  assert.match(
    html,
    /class="id-example" aria-label="Step 1\. Turn it over\. Show the back of the ID\."[\s\S]*?images\/mi-id-front-demo\.webp\?v=20260902-28" width="640" height="404" alt=""[\s\S]*?id="step1Title"[\s\S]*?<span class="step-number" aria-hidden="true">1<\/span>[\s\S]*?<strong>Turn it over<\/strong><small>Show the back<\/small>/
  );
  assert.match(
    html,
    /class="id-example is-target" aria-label="Step 2\. Find the wide barcode\. It is the second barcode from the top on the right, directly below the thin barcode\."[\s\S]*?images\/mi-id-back-demo\.webp\?v=20260902-28" width="640" height="404" alt=""[\s\S]*?id="step2Title"[\s\S]*?<span class="step-number" aria-hidden="true">2<\/span>[\s\S]*?<strong>Find the wide barcode<\/strong><small>Second from top, on the right<\/small>/
  );
  assert.match(
    html,
...
test("demo ID artwork is lightweight and clearly non-document training media", () => {
  ...
  assert.match(html, /mi-id-front-demo\.webp\?v=20260902-28" width="640" height="404"/);
  assert.match(html, /mi-id-back-demo\.webp\?v=20260902-28" width="640" height="404"/);
});
...
test("scanner asset versions are updated together", () => {
  ...
  assert.equal(cssVersion, "20260902-28");
  assert.equal(scriptVersion, cssVersion);
  assert.match(html, /mi-id-front-demo\.webp\?v=20260902-28/);
  assert.match(html, /mi-id-back-demo\.webp\?v=20260902-28/);
  assert.match(pairingJs, /new URLSearchParams\(\{ s: sessionId, k: key \}\)/);
  assert.match(pairingJs, /cb=20260902-28#\$\{fragment\.toString\(\)\}/);
