...
export function problemTitleBrands(title) {
  ...
  // Match whole markers only: a phrase mentioning "no brand" can still
  // contain a reported SALVAGE or other brand that must remain visible.
  const noBrand = /^(?:clean|unknown|undefined|none|no brands?(?: were returned(?: for this vehicle)?)?\.?)$/i;
  return reported.filter((brand) =>
    typeof brand === "string" && brand.trim() && !noBrand.test(brand.trim())
  );
}
