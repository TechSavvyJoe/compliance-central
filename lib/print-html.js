...
/**
 * Page geometry and the type scale, in points, for both renderers.
 *
 * The CSS custom properties below are generated from these numbers and the
 * jsPDF download path reads the very same object, so the printed sheet and the
 * downloaded PDF cannot drift into two different documents again. Points are
 * the unit throughout: a PDF has no other, and what is measured in points
 * prints at the same size whatever DPI the driver reports.
 */
export const PRINT_METRICS = Object.freeze({
  /** 0.6in, the one @page margin, expressed the way jsPDF wants it. */
  margin: 43.2,
  /** Letter portrait, less the margin on both sides. */
  contentWidth: 612 - 43.2 * 2,
  pageWidth: 612,
  pageHeight: 792,
  /**
   * Six sizes. The floor is 10pt — a customer keeps these records, and the
   * running head, the column heads and the footer are as readable as the body.
   * Each step is the smallest one that still separates two levels.
   */
  type: Object.freeze({
    caption: 10,
    body: 11,
  ...
  /** 140% of size for prose, tight enough for display sizes not to gap. */
  leading: Object.freeze({ body: 1.4, display: 1.15 }),
  /** Three rule weights, each with one job. */
  rule: 0.75,
  ruleAccent: 1.5,
  ruleHeavy: 2.25,
  radius: 2,
...
export function printBaseCSS() {
  ...
  return `
    :root {
      ...
      /* Six type sizes, from lib/print-html.js PRINT_METRICS — the same
         numbers the downloaded PDF is drawn from. Nothing on paper is set
         below 10pt: a customer keeps these records, so the running head, the
         column heads and the footer are as readable as the body copy. */
      --t-caption: ${m.type.caption}pt; --t-body: ${m.type.body}pt;
      --t-lead: ${m.type.lead}pt; --t-verdict: ${m.type.verdict}pt;
      --t-masthead: ${m.type.masthead}pt; --t-hero: ${m.type.hero}pt;
      --lh-body: ${m.leading.body}; --lh-display: ${m.leading.display};
      ...
      --rule: ${m.rule}pt; --rule-accent: ${m.ruleAccent}pt;
      --rule-heavy: ${m.ruleHeavy}pt; --radius: ${m.radius}pt;
      /* Six-step spacing scale. */
      --s1: ${m.s1}pt; --s2: ${m.s2}pt; --s3: ${m.s3}pt;
      --s4: ${m.s4}pt; --s5: ${m.s5}pt; --s6: ${m.s6}pt;
    }
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* A reference — VIN, licence number, plate — reads one glyph at a time.
       It takes the size of whatever it sits in rather than carrying its own,
       so a reference can never fall under the 10pt floor. */
    .ref {
    ...
    .main-title {
      ...
      font-size: var(--t-masthead); font-weight: 700; line-height: var(--lh-display);
    ...
    /* --- Dealership masthead ---------------------------------------- *
     * The store's logo and name lead the sheet and the record's own title
     * follows, so a screening record reads as the dealership's paperwork
     * rather than the app's. Both halves are optional: with no dealership
     * configured the documents fall back to .main-title and this never
     * renders. The logo is capped in height so an oversized upload cannot
     * push the record down the page. */
    .brand-masthead {
      display: flex; align-items: flex-end; gap: var(--s4);
      break-after: avoid; page-break-after: avoid;
    }
    .brand-logo { flex: none; max-height: 0.5in; max-width: 2in; object-fit: contain; }
    ...
    .brand-title {
      ...
      font-size: var(--t-masthead); font-weight: 700;
      line-height: var(--lh-display); letter-spacing: -0.015em;
    }
    .doc-sub { margin: 0 0 var(--s4); color: var(--slate); font-size: var(--t-body); }
    ...
    .app-notice {
      margin: 0 0 var(--s3); color: var(--alert);
      font-size: var(--t-caption); font-weight: 700; letter-spacing: .09em; text-transform: uppercase;
    }
    ...
    .page-header {
      ...
      color: var(--slate); font-size: var(--t-caption); line-height: 1.35;
    }
    .page-header > div { min-width: 0; overflow-wrap: break-word; }
    ...
    /* --- One heading scale ----------------------------------------- *
     * Two levels below the masthead, and each is bumped by the smallest
     * visible increment: not size, but weight, case, tracking and navy.
     * Both keep with the text they introduce. */
    h2, h3, .section-title, .brands-title, .check-summary-title, .results-header {
      margin: var(--s4) 0 var(--s2);
      color: var(--navy); font-size: var(--t-body); font-weight: 700;
      letter-spacing: .09em; text-transform: uppercase;
      break-after: avoid; page-break-after: avoid;
    }
    .section-subtitle { margin: calc(-1 * var(--s1)) 0 var(--s4); color: var(--slate); font-size: var(--t-body); }
    ...
    /* Money is read down the column, so it is set against the right edge on a
       common decimal rather than ranged left behind labels of varying length. */
    th.amount, td.amount {
      width: 1.5in; text-align: right; white-space: nowrap;
    ...
    tr.total th, tr.total td {
      ...
      border-bottom: var(--rule) solid var(--line);
      color: var(--navy); font-size: var(--t-lead); font-weight: 700;
    }
    ...
    .certification, .summary-notice, .note {
      /* Prose is inset far enough that a full-width card still holds a line
         under 90 characters, which is where a paragraph stops being scannable. */
      margin: var(--s3) 0; padding: var(--s3) var(--s5);
      border: var(--rule) solid var(--line); border-left: var(--rule-heavy) solid var(--slate);
      border-radius: var(--radius); background: var(--paper);
      color: var(--ink); font-size: var(--t-body); line-height: var(--lh-body);
    ...
    .result h2, .overall-decision strong, .eligible-text strong {
      ...
      line-height: var(--lh-display); letter-spacing: -0.01em; text-transform: none;
    }
    /* A verdict is read before it is understood, so it carries a glyph in its
       own status colour: a check when the check cleared, an exclamation when
       it did not. The glyph repeats the outcome rather than adding a signal. */
    .result h2, .overall-decision strong { display: flex; align-items: center; gap: var(--s2); }
    .verdict-icon { width: 14pt; height: 14pt; flex: none; fill: currentColor; }
    /* The decision word is navy on an approved jacket, so its check takes the
       success green rather than borrowing the heading's colour. */
    .decision-approved .verdict-icon { fill: var(--ok); }
    .decision-review .verdict-icon { fill: var(--gold); }
    .decision-denied .verdict-icon { fill: var(--alert); }
    ...
    .summary-notice strong {
      margin: 0 0 var(--s1); color: var(--navy); font-size: var(--t-body);
      letter-spacing: .08em; text-transform: uppercase;
    }
    .summary-notice span { color: var(--slate); font-size: var(--t-body); }

    /* --- Footers ---------------------------------------------------- */
    .footer, .portal-footer {
      margin: var(--s4) 0 0; padding-top: var(--s3);
      border-top: var(--rule) solid var(--line);
      color: var(--slate); font-size: var(--t-caption); line-height: 1.4; text-align: center;
    }
    .footer p, .portal-footer p { margin: 0; }
    /* Page numbering closes the footer, centred, so a jacket that has come
       apart can be put back in order. It is written into the document because
       Chrome resolves neither counter(page) nor an @page margin box. */
    .page-count {
      display: block; margin-top: var(--s1); color: var(--slate);
...
export function reportDocumentCSS() {
  return `${printBaseCSS()}
    ...
    .page {
      /* The footer and its page number are pinned to the bottom of the sheet,
         so the flow stops clear of them rather than running underneath. One
         footer line plus the page number needs 0.55in; the OFAC record, which
         also names its data source, needs a line more. */
      width: 100%; min-height: 9.7in; position: relative; padding-bottom: 0.55in;
      break-after: page; page-break-after: always;
    }
    .page.ofac-page { padding-bottom: 0.72in; }
    ...
    /* --- Subject panel ---------------------------------------------- */
    .subject, .content-box {
      /* Tighter top and bottom than the sides: a panel of one-line rows needs
         less air above the first than beside it, and a branded masthead has
         already taken a fifth of an inch off every sheet. */
    ...
    /* --- What was screened ------------------------------------------- *
     * Four lines of plain statement, so a cleared record says which list was
     * searched, when it was retrieved and how close a name had to be — a
     * parameter table said the same thing and took an inch and a half of a
     * sheet that also has to hold the verdict and the certification. */
    .screening-scope strong { color: var(--navy); }
    ...
    .matches {
      ...
      border-top: var(--rule) solid var(--line);
      color: var(--ink); font-size: var(--t-body); text-align: left;
    }
    .matches ul { margin: var(--s2) 0 0 var(--s4); }
    .matches em { color: var(--slate); font-style: italic; }

    /* --- MDOS-shaped summary fields ----------------------------------- */
    .form-grid { display: grid; gap: var(--s2) var(--s4); margin-bottom: var(--s3); }
    ...
    .form-label { margin-bottom: var(--s1); color: var(--slate); font-size: var(--t-caption); font-weight: 400; }
    .form-value {
      ...
      font-size: var(--t-body); font-weight: 600; text-transform: uppercase; line-height: 1.25;
    ...
    .detail-row {
      ...
      padding: var(--s1) 0; border-bottom: var(--rule) solid var(--line); font-size: var(--t-body);
      break-inside: avoid; page-break-inside: avoid;
    }
    ...
    .vin-search-info { margin: 0 0 var(--s4); color: var(--slate); font-size: var(--t-body); }
    .vin-search-info strong { color: var(--ink); }

    /* --- Eligible / review card --------------------------------------- */
    .eligible-card { display: flex; gap: var(--s3); align-items: flex-start; }
    .eligible-icon { width: 13pt; height: 13pt; fill: currentColor; flex-shrink: 0; margin-top: 2pt; }
    .eligible-card { color: var(--ok); }
    .eligible-card.result-review { color: var(--navy); }
    .eligible-text { color: var(--ink); font-size: var(--t-body); line-height: var(--lh-body); }
    .eligible-note { margin-top: var(--s2); color: var(--slate); font-size: var(--t-caption); font-weight: 400; }
    ...
    .state-evidence p { margin: 0; color: var(--slate); font-size: var(--t-caption); line-height: 1.4; }
    .state-evidence-part { flex: 0 0 auto; color: var(--slate); font-size: var(--t-caption); font-weight: 700; white-space: nowrap; }
