...
/**
 * Turn a failed response into something a salesperson can act on.
 *
 * This message reaches a toast verbatim. It used to read "Backend error: HTTP
 * 503" — "backend" is our word for our own server, and a status code is not a
 * fact anyone at a desk can do anything with. The service curates its own error
 * text, so that is preferred when it is present and sane; the fallback now says
 * what happened and whether waiting will help.
 */
export function httpFailureMessage(status, serverMessage) {
  const supplied = String(serverMessage ?? "").trim();
  // A stack trace or a runaway string is the service leaking its internals into
  // the dealer's screen, so it is not passed through.
  const usable =
    supplied &&
    supplied.length <= 200 &&
    !/\n/.test(supplied) &&
    !/\b(at\s+\w+\s+\(|Error:|TypeError|undefined is not)/.test(supplied);
  if (usable) return supplied;

  if (status === 429) {
    return "The compliance service is busy right now. Wait a moment and try again.";
  }
  if (status === 502 || status === 503 || status === 504) {
    return "The compliance service is temporarily unavailable. Try again in a moment.";
  }
  if (status >= 500) {
    return "The compliance service could not complete this check. Try again.";
  }
  if (status === 404) {
    return "The compliance service could not find that record.";
  }
  if (status >= 400) {
    return "The compliance service could not accept this request. Check the details and try again.";
  }
  return "The check did not complete. Try again.";
}
    ...
    for (let attempt = 0; ; attempt++) {
      ...
      try {
        ...
        if (error.name === "AbortError") {
          ...
          // "Backend" is our word for our own server. The sibling message two
          // lines down already calls it the compliance service, and this one
          // can reach a toast verbatim through describeError().
          throw new Error(
            "The compliance service did not respond in time. Please try again."
          );
        }
      ...
      if (!response.ok) {
        ...
        throw new Error(httpFailureMessage(response.status, errorData.error));
      }
    ...
    quote: {
      ...
      recreationPassport:
        quote.recreationPassport === true || quote.recreationPassport === false
          ...
          : null,
      // How long the plate runs and the day it lapses. Optional — a fee that
      // reconciled is a valid quote without them — but a malformed value is
      // dropped rather than shown, because a wrong expiry date on a customer's
      // paperwork is worse than no date at all.
      registrationMonths:
        Number.isInteger(quote.registrationMonths) &&
        quote.registrationMonths > 0 &&
        quote.registrationMonths <= 24
          ? quote.registrationMonths
          : null,
      expiresOn:
        typeof quote.expiresOn === "string" &&
        /^\d{4}-\d{2}-\d{2}$/.test(quote.expiresOn) &&
        !Number.isNaN(new Date(`${quote.expiresOn}T00:00:00Z`).getTime())
          ? quote.expiresOn
          : null,
      officialPageImage:
