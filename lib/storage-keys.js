/**
 * Centralised storage keys and status enums shared by worker and sidepanel.
 *
 * Any change here must be coordinated across the consumers — keep this file
 * the single source of truth.
 */

export const STORAGE_KEYS = Object.freeze({
  // Per-run state (use chrome.storage.session; do not persist customer data)
  currentResults: "currentResults",
  searchStatus: "searchStatus",
  searchProgress: "searchProgress",
  inFlightCheck: "inFlightCheck", // which MDOS check is currently running
  lastError: "lastError",
  activeRunId: "activeRunId",
  stateRunId: "stateRunId",
  cancelledRunId: "cancelledRunId",
  activeIndividualOperationId: "activeIndividualOperationId",
  cancelledIndividualOperationId: "cancelledIndividualOperationId",

  // Screenshots (transient between worker write and sidepanel read; session only)
  repeatOffenderScreenshot: "repeatOffenderScreenshot",
  coBuyerRepeatOffenderScreenshot: "coBuyerRepeatOffenderScreenshot",
  titleScreenshot: "titleScreenshot",

  // Persistent
  complianceHistory: "complianceHistory",
  searchHistory: "searchHistory", // legacy per-check feed
  backendApiKey: "backendApiKey", // legacy only; removed during Settings init
  rescreenReminderEnabled: "rescreenReminderEnabled",
  lastResult: "lastResult", // session only

  // Session-only
  cachedFormData: "cachedFormData",
  cachedAt: "cachedAt",
  sosFeeQuote: "sosFeeQuote",
  // Extension-owned inactive SOS calculator tab ID only. It contains no
  // calculator fields and is cleared by the worker on restart/close.
  sosFeeActiveTabId: "sosFeeActiveTabId",
});

export const SEARCH_STATUS = Object.freeze({
  idle: "idle",
  running: "running",
  complete: "complete",
  error: "error",
});

// Keys used for `inFlightCheck` so sidepanel knows which row to mark "Running".
export const IN_FLIGHT = Object.freeze({
  ofac: "ofac",
  coBuyerOfac: "coBuyerOfac",
  repeatOffender: "repeatOffender",
  coBuyerRepeatOffender: "coBuyerRepeatOffender",
  title: "title",
});
