...
export function imageDataUrlExtension(data) {
  const safe = ensureDataUrl(data);
  if (!safe) return null;
  if (/^data:image\/jpe?g;/i.test(safe)) return "jpg";
  if (/^data:image\/webp;/i.test(safe)) return "webp";
  return "png";
}
