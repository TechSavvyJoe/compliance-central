...
function drawSubjectBox(ctx, opts) {
  ...
  const preparedRows = rows.map((row) => {
    ...
    return {
      ...
      height: Math.max(22, lines.length * rowLineHeight + 8),
    };
  });
...
function drawScreenshotPage(ctx, dataUrl, opts = {}) {
  ...
  const bottom = Number.isFinite(opts.bottom)
    ? opts.bottom
    : pageHeight - margin - (opts.reserveFooter ? 36 : 0);
...
function drawPortalCapture(ctx, opts) {
  ...
  if (metaLine) {
    doc.setFontSize(7.5);
    const lines = doc.splitTextToSize(String(metaLine), pageWidth - evidenceMargin * 2);
    doc.text(lines.slice(0, 2), evidenceMargin, ctx.y + 7);
    ctx.y += Math.min(2, lines.length) * 9;
  }

  setDraw(doc, PALETTE.border);
  doc.setLineWidth(0.5);
  doc.line(evidenceMargin, ctx.y + 2, pageWidth - evidenceMargin, ctx.y + 2);
  ctx.y += 8;

  drawScreenshotPage(ctx, screenshot, {
    margin: evidenceMargin,
    bottom: pageHeight - 29,
