...
1. `npm test` — all tests green (334 passing as of this writing).
...
| Order | File | Caption (one line) | What the image must show |
|---|---|---|---|
| 1 | `screenshot-1.png` | Run every check from one panel — one clearly labeled verdict per deal | The redesigned 1.6.0 side panel after Run All Checks: buyer and co-buyer fields filled with obviously fictional sample data, each check's labeled outcome, and the overall verdict. |
| 2 | `screenshot-2.png` | OFAC screening runs on this computer, against the local Treasury SDN list | An OFAC-only result with the on-device/privacy note and list-freshness state visible. |
| 3 | `screenshot-3.png` | Title & Lien: what the Michigan portal returned, with the captured state page as evidence | A title/lien outcome including the captured MDOS page evidence block. |
| 4 | `screenshot-4.png` | Scan a license with your phone — encrypted transfer, the image stays on the phone | The phone pairing/scan flow. **Must keep the visible "Instructional composite · Phone scan" label and fictional ID artwork** — it is not a live capture of a real document (pinned by `tests/release-copy.test.js`). |
| 5 | `screenshot-5.png` | Saved records: reopen, re-screen, print, or delete — device-local, 30 days / 50 records | The History tab with per-record Re-screen and Delete buttons on a card; fictional names and record ids (e.g. CC-20260722-091421). |
...
| 8 | Host justification — `compliance-central-api.fly.dev` | The extension's backend: MDOS Repeat Offender + Title/Lien checks, server-driven SOS fee-calculator runs, and the encrypted single-use phone-scan relay it cannot read. In-memory processing; no server-side retention; IP used transiently for rate limiting only. (Full text in `privacy-tab.txt`.) |
